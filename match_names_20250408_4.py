...
# Uncomment the following lines (only once) if you haven't downloaded these:
# import nltk
# nltk.download('stopwords')
# nltk.download('wordnet')

nltk_stopwords = set(stopwords.words("english"))
# You can remove or add custom stopwords if desired; here we remove "ripe" as an example.
...
def normalize_token(token, alias_map=None):
    """
    Lowercase, remove punctuation, lemmatize and then apply custom plural override.
    If an alias_map is provided and the token exists in it, return the mapped value.
    """
    ...
    if alias_map and token in alias_map:
        # Assuming alias_map maps a token to a tuple (canonical_token, weight)
...
def embed_text_gensim(text, ft_model):
    # Tokenize, normalize, and lemmatize the text before embedding.
...
def build_token_to_usda_index(usda_rows):
    """
    Build a mapping from each token (normalized) in canonical_name + modifiers → set of USDA ids.
    """
...
def store_usda_sub_embedding(usda_id, sub_label, sub_text, embedding, weight=1.0):
    """
    Insert a row into the usda_sub_embeddings table for a token- or phrase-based embedding.
    """
...
def store_usda_full_embedding(usda_id, embedding):
    """
    Upsert the full ingredient phrase embedding in usda_ingredient_embeddings.
    """
    float_list = [float(x) for x in embedding]
    conn = psycopg2.connect(host=DB_HOST, database=DB_NAME, user=DB_USER,
                            password=DB_PASSWORD, port=DB_PORT)
...
def get_combined_usda_embedding(usda_id):
    """
    Combines all sub-embeddings for a given USDA ingredient (weighted) and L2-normalizes.
    """
    conn = psycopg2.connect(host=DB_HOST, database=DB_NAME, user=DB_USER,
                            password=DB_PASSWORD, port=DB_PORT)
    cur = conn.cursor()
...
def build_usda_full_name_v2(canonical_name, modifiers):
    """
    Concatenate canonical name with its modifiers.
    """
...
def candidate_filter_with_tokens(local_tokens, token_index, alias_map=None):
    ...
    for token in local_tokens:
        normalized_token = normalize_token(token, alias_map)
        candidate_ids |= token_index.get(normalized_token, set())
...
#############################
def hybrid_match_ingredient(local_string, ft_model, token_index, usda_text_map, alias_map=None, penalty_factor=3):
    # Tokenize and normalize the local string.
    ...
    for usda_id in candidate_ids:
        ...
        # Compute fuzzy score (scale: 0-100)
        fuzzy_score = fuzz.token_set_ratio(local_string, candidate_text)
        
        # Compute token sets for penalty deduction
        candidate_tokens = set(candidate_text.split())
        extra_tokens = candidate_tokens - set(local_string.split())
        penalty = len(extra_tokens) * penalty_factor
        
        adjusted_fuzzy_score = max(fuzzy_score - penalty, 0)
        
        embedding = get_combined_usda_embedding(usda_id)
        if embedding is None:
            continue
        embedding_score = cosine_similarity(local_vec, embedding)
        
        combined_score = 0.7 * embedding_score + 0.3 * (adjusted_fuzzy_score / 100.0)
...
def aggregate_usda_matches_for_recipe(min_id, max_id, ft_model, faiss_index, usda_ids, token_index, usda_text_map, embedding_threshold=0.75):
    ...
    for row in recipe_rows:
        ...
        # Here you might incorporate local sub-embeddings if available, otherwise fallback:
    ...
    # Example using FAISS matching on local embeddings (can be combined with the hybrid match as needed)
...
#############################
#   1) Build & Store Sub-Embeddings for USDA Items (with Overrides)
#############################
def build_all_usda_sub_embeddings(ft_model, default_canonical_weight=2.0, default_modifier_weight=1.0, default_combined_weight=1.0):
    """
    For each USDA item, parse canonical name and modifiers.
    Check for override weights from usda_weight_overrides.
    Store sub-embeddings in usda_sub_embeddings.
    Also store entire phrase embedding in usda_ingredient_embeddings.
    """
...
if __name__ == "__main__":
    ...
    # 1) Build sub-embeddings using override logic
    ...
    # 2) Build token index and full-text mapping from USDA ingredients
