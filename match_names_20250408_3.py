...
def normalize_token(token, alias_map=None, idf_weights=None, idf_threshold=1.5):
    """
    Normalize token by lowercasing, stripping punctuation, lemmatizing, and applying any alias mapping.
    Additionally, if an IDF weight is provided, optionally reduce the token’s influence or
    mark it as less important if the token is very common (i.e. low IDF).
    """
    token = re.sub(r'[^\w\s]', '', token.lower().strip())
    token = lemmatizer.lemmatize(token)
    token = custom_plural_override.get(token, token)
    # If an alias mapping is provided, use it.
    if alias_map is not None and token in alias_map:
        token = alias_map[token][0]
    # Optionally check the IDF weight for this token.
    if idf_weights is not None:
        # If the token is extremely common (IDF lower than threshold), you may choose to
        # return an empty string (i.e., filter it out) or tag it as less important.
        if token in idf_weights and idf_weights[token] < idf_threshold:
            # Option 1: Return an empty string to filter it out:
            return ""
            # Option 2: Alternatively, you might adjust the token somehow.
    return token
...
#   EMBEDDING HELPERS
#############################
def embed_text_gensim(text, ft_model, alias_map=None):
    # Tokenize, normalize (with alias mapping if provided) the text.
    tokens = [normalize_token(t, alias_map) for t in text.split()]
...
def build_token_to_usda_index(usda_rows, alias_map=None):
    """
    ...
    for uid, cname, mods in usda_rows:
        tokens = [normalize_token(t, alias_map) for t in cname.split()]
        tokens += [normalize_token(m, alias_map) for m in mods if m]
        for tok in tokens:
            if tok:
                index[tok].add(uid)
    return index

def compute_idf_weights(usda_rows):
    """
    Build a TF-IDF vectorizer on the USDA ingredients' full text (canonical_name plus modifiers)
    and return a dictionary mapping each token to its IDF weight.
    """
    # Build a list of full texts
    documents = []
    for uid, canonical, mods in usda_rows:
        full_text = canonical.lower().strip() + " " + " ".join(m.lower() for m in mods if m)
        documents.append(full_text)
        
    vectorizer = TfidfVectorizer(stop_words='english')  # you can supply your own stop_words list if desired
    vectorizer.fit(documents)
    
    # The idf values are stored in vectorizer.idf_ and the vocabulary in vectorizer.vocabulary_
    idf_weights = {}
    for token, idx in vectorizer.vocabulary_.items():
        idf_weights[token] = vectorizer.idf_[idx]
...
def get_combined_usda_embedding(usda_id):
    ...
    (Combined full-phrase embedding is not used in this test.)
    """
    conn = psycopg2.connect(host=DB_HOST, database=DB_NAME, user=DB_USER,
                              password=DB_PASSWORD, port=DB_PORT)
    cur = conn.cursor()
...
def build_usda_full_name_v2(canonical_name, modifiers):
    """
    Concatenate the canonical name and its modifiers.
    """
...
#############################
def candidate_filter_with_tokens(local_tokens, token_index, alias_map=None):
    candidate_ids = set()
    for token in local_tokens:
        token_norm = normalize_token(token, alias_map)
        candidate_ids |= token_index.get(token_norm, set())
    return candidate_ids

def match_ingredient_token_filtered(local_string, ft_model, token_index, alias_map=None):
    local_tokens = set([normalize_token(t, alias_map) for t in local_string.split()])
    candidate_ids = candidate_filter_with_tokens(local_tokens, token_index, alias_map)
    if not candidate_ids:
        return None, 0.0

    local_vec = embed_text_gensim(local_string, ft_model, alias_map)
...
#############################
def hybrid_match_ingredient(local_string, ft_model, token_index, usda_text_map, alias_map=None):
    local_tokens = set([normalize_token(t, alias_map) for t in local_string.split()])
    candidate_ids = candidate_filter_with_tokens(local_tokens, token_index, alias_map)
    if not candidate_ids:
        return None, 0.0
...
#   Aggregator Example
#############################
def aggregate_usda_matches_for_recipe(min_id, max_id, ft_model, faiss_index, usda_ids, token_index, usda_text_map, alias_map=None, embedding_threshold=0.75):
    ...
    for row in recipe_rows:
        ...
        local_vec = embed_text_gensim(local_name, ft_model, alias_map)
...
def build_all_usda_sub_embeddings(ft_model, default_canonical_weight=2.0, default_modifier_weight=1.0):
    ...
    (Combined full-phrase embeddings are not stored in this test.)
    """
...
if __name__ == "__main__":
    ...
    # Load token-level alias mappings from usda_aliases
    alias_map = load_usda_aliases()
    ...
    # 2) Build token index and full-text mapping (based solely on canonical name and modifiers)
    usda_rows = get_usda_ingredients_with_modifiers()
    token_index = build_token_to_usda_index(usda_rows, alias_map)
    ...
    test_strings = [
        ...
        "zucchini medium",
        "banana",
        "flour g",               # measurement token should be removed via normalization/aliasing
    ...
    for s in test_strings:
        best_id, score = hybrid_match_ingredient(s, ft_model, token_index, usda_text_map, alias_map)
        if best_id:
