...
#############################
#   1) Build & Store sub-embeddings for all USDA items (Tokens + Combined)
#############################
def build_all_usda_sub_embeddings(ft_model, weight_canonical=2.0, weight_modifier=1.0, weight_combined=1.0):
    ...
    for (uid, cname, mods) in usda_data:
        ...
        # 3) combined entire text
        filtered_mods = [m for m in mods if m]  # skip None/empty
        full_text = cname + " " + " ".join(filtered_mods)

        combined_vec = embed_text_gensim(full_text, ft_model)
        if combined_vec is not None:
            store_usda_sub_embedding(uid, "combined", full_text, combined_vec, weight=weight_combined)

        count += 1

    logging.info(f"Built sub-embeddings for {count} USDA items (tokens + combined).")
...
if __name__ == "__main__":
    ...
    # 1) Build sub-embeddings for all USDA items: tokens + combined
    build_all_usda_sub_embeddings(ft_model, weight_canonical=2.0, weight_modifier=1.0, weight_combined=1.0)
    ...
    # 3) Test the approach with some strings
