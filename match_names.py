...
def hybrid_match_ingredient(local_string, ft_model, token_index, usda_text_map):
    """
    First filters candidates by token overlap; then for each candidate, it calculates a fuzzy string
    similarity score (using RapidFuzz) between the local string and the USDA full text.
    It also computes the embedding cosine similarity and finally combines these scores (here we simply
    take an average of the normalized fuzzy score and the cosine similarity).
    """
    ...
    best_id = None
    best_score = 0.0
    # Iterate over candidates and compute combined score
    for usda_id in candidate_ids:
        candidate_text = usda_text_map.get(usda_id, "")
        fuzzy_score = fuzz.token_set_ratio(local_string, candidate_text)  # 0-100
        embedding = get_combined_usda_embedding(usda_id)
        if embedding is None:
            continue
        embedding_score = cosine_similarity(local_vec, embedding)  # 0-1
        # Combine the fuzzy score (normalized to 0-1) and embedding similarity
        combined_score = 0.3 * embedding_score + 0.7 * (fuzzy_score / 100.0)
