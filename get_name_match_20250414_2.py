...
def batch_match_stored_embeddings(local_subs, usda_subs, ft_model=None):
    conn = get_db_connection()
    stop_chunks = fetch_stop_chunks()
    antonyms_dict = fetch_antonyms()
    domain_weights = fetch_domain_weights()
    conn.close()
    ...
    for local_id, loc_list in local_subs.items():
        ...
        best_sc=-1.0
        for uid in cands:
            sc = weighted_jaccard_stored_emb(
                loc_list, 
                usda_subs[uid],
                ft_model=ft_model,
            ...
            if sc>best_sc:
                best_sc=sc
                best_uid=uid
        results[local_id]=(best_uid,best_sc)
    return results
...
if __name__=="__main__":
    ...
    logging.info("Running batch match with domain-coded weighting, antonyms, stop-chunks, synonyms.")
    ...
    # print top 10
    for i,(uid,sc) in list(best_matches.items())[:10]:
        lname=local_names.get(i,"?")
        if uid:
            uname=usda_names.get(uid,"?")
            logging.info(f"Local {i} '{lname}' => USDA {uid} '{uname}', WeightedJaccard={sc:.2f}")
        else:
            logging.info(f"Local {i} '{lname}' => no match found, sc=0.0")
    logging.info("Done.")
